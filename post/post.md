...
- *consultancy* — $A$ commits a class and shows the single mask that best supports it; the judge sees that mask and $d_A$. This is a protocol arm rather than a plain baseline: its judge is trained on-policy on its own plays, like the debate judges.
