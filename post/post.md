...
**Baselines.** Four baselines run on the same judge architecture as the arms. *Gold* is the judge given the full image with no mask at all. *Random mask (1)* is a single uniform 4/36 mask with no agent and no claim input, which is the sparse-judge floor. *Random mask (ensemble)* averages the judge's probabilities over $M$ random masks and returns the argmax, $\arg\max_c \frac{1}{M}\sum_m p[m,c]$. The tables report it at every mask budget the arms use, and again at $M = \binom{36}{4} = 58{,}905$. The matching budgets are the point: an arm and its ensemble row then see the same number of masks, so the difference between them is what *selecting* the masks buys — by search in consultancy, by minimax in the debates — over just averaging the judge across random ones. The all-masks row is the zero-capability endpoint of the protocol family from the main text. Finally, *consultancy* is the arm where $A$ commits a class and shows the single mask that best supports it, with the judge seeing the mask and $d_A$ and no adversary present.
