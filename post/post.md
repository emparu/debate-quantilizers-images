...
Finally, we checked that the search code really does behave this way. This one is a test of the implementation, so it runs on freshly initialised judges rather than on the trained ones: we take a channel judge, zero the head weights on its four bit inputs so that it cannot react to the symbols, and copy its remaining weights into a no-channel judge, so that the two compute the same function of the revealed cells. We then run the full (cell, symbol) search with the first — the 16 symbol sequences are still enumerated, they simply cannot change any score — and the plain cell-only search with the second, on the same sampled masks. The two produce the same play: same cells, same claim, same $d_B$, for every arm on both datasets.
