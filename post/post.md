...
**Baselines.** Four baselines run on the same judge architecture as the arms. *Gold* is the judge given the full image with no mask at all. *Random mask (1)* is a single uniform 4/36 mask with no agent and no claim input, which is the sparse-judge floor. *Random mask (ensemble)* draws a set of $M$ random masks, averages the judge's probabilities over them and takes the argmax, $\arg\max_c \frac{1}{M}\sum_m p[m,c]$. We report it at two kinds of $M$. The first is each mask budget an arm actually searches (144, 256, 324 or 1296), and those rows are the informative comparison, because they ask what a fixed number of judge evaluations buys: an agent that *searches* 256 masks for the most convincing one, against simply *averaging* 256 random ones. The second is all $\binom{36}{4} = 58{,}905$ masks, which is the zero-capability endpoint of the protocol family from the main text. Finally, *consultancy* is the arm where $A$ commits a class and shows the single mask that best supports it, with the judge seeing the mask and $d_A$ and no adversary present.
